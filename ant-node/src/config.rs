...
/// WebTransport listener configuration for browser client support.
///
/// When present, the node starts an HTTP/3 (QUIC) WebTransport server that
/// browsers can connect to using the W3C WebTransport API. A self-signed TLS
/// certificate is generated at startup; the SHA-256 hash is logged so browser
/// clients can pin it via `serverCertificateHashes`.
#[cfg(feature = "webtransport")]
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WebTransportConfig {
    /// Address for the HTTP/3 WebTransport server (e.g., `0.0.0.0:4433`).
