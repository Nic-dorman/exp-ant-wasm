...
pub struct DevnetManifest {
    ...
    /// WebTransport endpoint info (present when WebTransport is enabled).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub webtransport: Option<DevnetWebTransportInfo>,
}

/// WebTransport endpoint info included in the devnet manifest.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DevnetWebTransportInfo {
    /// WebTransport URL (e.g. `https://127.0.0.1:15001`).
    pub url: String,
    /// SHA-256 certificate hash (hex, no colons) for `serverCertificateHashes`.
    pub cert_hash: String,
    /// PeerId of the node serving WebTransport (hex, 32 bytes = 64 chars).
    /// Used as the trust anchor for ML-DSA-65 authenticated handshake.
    pub peer_id: String,
}
...
impl Devnet {
    ...
    /// Get the `AntProtocol` handler from the first (bootstrap) node.
    #[must_use]
    pub fn bootstrap_protocol(&self) -> Option<Arc<AntProtocol>> {
        self.nodes.first().and_then(|n| n.ant_protocol.clone())
    }

    /// Get the `P2PNode` from the first (bootstrap) node.
    #[must_use]
    pub fn bootstrap_p2p_node(&self) -> Option<Arc<P2PNode>> {
        self.nodes.first().and_then(|n| n.p2p_node.clone())
    }

    /// Get the `NodeIdentity` from the first (bootstrap) node.
    #[must_use]
    pub fn bootstrap_node_identity(&self) -> Option<Arc<NodeIdentity>> {
        self.nodes.first().and_then(|n| n.node_identity.clone())
    }

    /// Get a clone of the devnet shutdown token.
    #[must_use]
    ...
    async fn create_node(
        ...
        Ok(DevnetNode {
            ...
            node_identity: Some(Arc::new(identity)),
            is_bootstrap,
